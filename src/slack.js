...
const { getSlackUserId, toOxfordComma } = require('./utils');
    ...
    switch (type) {
      ...
      case 'requestReviewForAuthor':
        const requestedReviewers = toOxfordComma(
          payload['pull_request']['requested_reviewers'].map((v) => v['login'])
        );
        return [
      ...
      case 'merged':
        return [
          `:white_check_mark: ${sender} merged your pull request`,
          'DONE',
        ];
    }
  }
